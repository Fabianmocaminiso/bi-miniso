...
// ─── Sprint 11 — RRHH desde vista materializada mv_cc_rh ───────────────────────
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const chk = await query(
      "SELECT COUNT(*) AS n FROM miniso_dlh.public.mv_cc_rh WHERE anio = $1 AND mes = $2",
      [year, month]
    );
    let y = year, m = month, fallback = false;
    if (!chk[0] || Number(chk[0].n) === 0) {
      const mx = await query("SELECT anio, mes FROM miniso_dlh.public.mv_cc_rh ORDER BY anio DESC, mes DESC LIMIT 1", []);
      if (mx[0]) { y = Number(mx[0].anio); m = Number(mx[0].mes); fallback = true; }
    }

    const rows = await query("SELECT * FROM miniso_dlh.public.mv_cc_rh WHERE anio = $1 AND mes = $2", [y, m]);
